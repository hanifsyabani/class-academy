...
export default function Home() {
  ...
  useEffect(() => {
    async function getDatas() {
      try {
        const teacherData = await fetch("/api/teachers");
        const studentData = await fetch("/api/students");
        const data = await teacherData.json();
    ...
    <AppLayout>
      <div className="px-5 py-4">
                ...
                <div className="mt-20 ">
                  <div className="flex items-center gap-4">
                    ...
                    <h1 className="font-semibold text-primary">
                      Statistics Class Academy
                    </h1>
                  </div>
                  <div className="w-full flex justify-center gap-8 mt-10">
                    <div className="w-1/2">
                      <h2 className="font-semibold text-center text-primary mb-4">
                        Grafik Role
                      </h2>
                      <BarChart
                        data1={teachers.length}
                        data2={students.length}
                        title1="Teachers"
                        title2="Students"
                      />
                    </div>
        ...
        <div className="mt-20 text-center">
          <h2 className="font-bold">Dashboard Class Academy</h2>
          <p>Copyright © 2024</p>
        </div>
      </div>
    </AppLayout>
